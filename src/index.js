...
const TOO_SMALL_TIME_FOR_WAITING = MINIMAL_WORKER_TIME -
  TESTCAFE_CLOSING_TIMEOUT;
...
function doRequest(apiPath, params) {
  ...
  if (params) url += "?" + makeQueryString(params);
...
export default {
  ...
  _getConnector() {
    this.connectorPromise = this.connectorPromise.then(async connector => {
      if (!connector)
        connector = await createBrowserStackConnector(
          process.env["BROWSERSTACK_ACCESS_KEY"]
        );
  ...
  _disposeConnector() {
    this.connectorPromise = this.connectorPromise.then(async connector => {
      if (connector) await destroyBrowserStackConnector(connector);
  ...
  _filterPlatformInfo(query) {
    return this.platformsInfo.filter(info => {
      var browserNameMatched = info["browser"] &&
        info["browser"].toLowerCase() === query.name;
      var deviceNameMatched = info["device"] &&
        info["device"].toLowerCase() === query.name;

      var browserVersionMatched = info["browser_version"] &&
        Number(info["browser_version"]) === Number(query.version);
      var platformVersionMatched = info["os_version"] &&
        Number(info["os_version"]) === Number(query.version);
      var platformNameMatched = info["os"].toLowerCase() === query.platform ||
        `${info["os"].toLowerCase()} ${info["os_version"].toLowerCase()}` ===
          query.platform;
  ...
  async openBrowser(id, pageUrl, browserName) {
    ...
    this.workers[id] = await doRequest(
      BROWSERSTACK_API_PATHS.newWorker,
      capabilities
    );
  ...
  async resizeWindow(/* id, width, height, currentWidth, currentHeight */) {
    this.reportWarning(
      'The window resize functionality is not supported by the "browserstack" browser provider.'
    );
  },

  async takeScreenshot(id, screenshotPath) {
    return new Promise(async (resolve, reject) => {
      var buffer = await doRequest(
        BROWSERSTACK_API_PATHS.screenshot(this.workers[id].id)
      );
